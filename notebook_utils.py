...
def merge_two_quantized_sequence_for_ploting(quantized_sequence1, quantized_sequence2):
    '''changed the note_sequence'''
    sequences_lib.assert_is_relative_quantized_sequence(quantized_sequence1)
    sequences_lib.assert_is_relative_quantized_sequence(quantized_sequence2)


    for note1 in quantized_sequence1.notes:
        note1.instrument = 0
        note1.program = 0

    for note2 in quantized_sequence2.notes:
        note2.instrument = 1
        note2.program = 1

        note = quantized_sequence1.notes.add()
        note.start_time = note2.start_time
        note.quantized_start_step = note2.quantized_start_step
        note.pitch = note2.pitch
        note.velocity = note2.velocity
        note.instrument = note2.instrument
        note.program = note2.program
        note.end_time = note2.end_time
        note.quantized_end_step = note2.quantized_end_step

    return quantized_sequence1
...
def plot_quantized_sequence(sequence,
                  show_figure=True):
  ...
  """
  sequences_lib.assert_is_relative_quantized_sequence(sequence)
  ...
  def _sequence_to_pandas_dataframe(sequence):
    ...
    for note in sequence.notes:
      pd_dict['quantized_start_step'].append(note.quantized_start_step)
      pd_dict['quantized_end_step'].append(note.quantized_end_step)
      pd_dict['duration'].append(note.quantized_start_step - note.quantized_end_step)
  ...
  fig.plot_width = 500*4
  fig.plot_height = 200*4
  fig.xaxis.axis_label = 'steps'
  ...
  for counter, instrument in enumerate(instruments):
    ...
    fig.quad(top='top', bottom='bottom',
             left='quantized_start_step', right='quantized_end_step',
...
def plot_sequence(sequence,
  ...
  fig = bokeh.plotting.figure(
      tools='hover,pan,box_zoom,reset,previewsave')
  fig.plot_width = 500*2
  fig.plot_height = 200*2
